...
async def get_roles_id_name():
    ...
    guild = await bot.client.fetch_guild(config['guild'])
...
async def get_guild_master_id(guild_id: str) -> str:
    return (await bot.client.fetch_guild(guild_id)).master_id
...
async def get_user(user_id: str) -> User:
    return await bot.client.fetch_user(user_id)
...
async def set_parent_id(msg: Message):
    ...
    if msg.ctx.guild.id != config['guild']:
        ...
        return
    parent_id = (await bot.client.fetch_public_channel(msg.ctx.channel.id)).parent_id
...
async def card_all_channel_send(msg: Message):
    ...
    if len(card_id) != 0:
        ...
        for i in config['channels']:
            await (await bot.client.fetch_public_channel(i)).send(card_msg)
